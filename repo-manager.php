...
/**
 * Plugin Name: Repo Manager for Git
...
if (! defined('GIT_MANAGER_DEBUG')) {
    define('GIT_MANAGER_DEBUG', false);
}
...
function git_manager_enqueue_global_checker($hook)
{
    ...
    if ($has_access) {
        ...
        if ($floating_widget_enabled) {
            ...
            wp_enqueue_style('repo-manager-floating-widget', GIT_MANAGER_URL . 'dist/css/floating-widget.css', [], GIT_MANAGER_VERSION);
            wp_enqueue_script('repo-manager-floating-widget', GIT_MANAGER_URL . 'dist/js/floating-widget.min.js', ['wp-i18n'], GIT_MANAGER_VERSION, true);
